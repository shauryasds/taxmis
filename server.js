  ...
  db.connect((err) => {
    ...
    db.query(
      ...
      CREATE TABLE IF NOT EXISTS leads (
          ...
          clientmobilenumber VARCHAR(255),
...
app.get("/", (req, res) => {
  ...
  res.sendFile(
    path.join(__dirname, "public", "Frontend-website", "index.html")
  );
});

app.get("/login", (req, res) => {
  console.log(path.join(__dirname, "public", "admin", "login.html"));
...
app.get("/convert-customer/:id", requireAuth, (req, res) => {
  res.sendFile(
    path.join(__dirname, "public", "admin", "convert-customer.html")
  );
});
app.get("/details-customer/:id", requireAuth, (req, res) => {
  res.sendFile(
    path.join(__dirname, "public", "admin", "details-customer.html")
  );
});
...
app.post("/update/:id",requireAuth,  async (req, res) => {
  ...
  try {
    const leadData = {
      customer_name: customer.customerName,
      mobile: customer.mobile,
      father_name: customer.fatherName || null,
      spouse_name: customer.spouseName || null,
      mother_name: customer.motherName || null,
      ...
      clientmobilenumber: customer.clientMobile || null,
      clientemail: customer.clientEmail || null,
    ...
    if (isCustomer) {
      const [lead] = await new Promise((resolve, reject) => {
        db.query(
          "SELECT documents FROM leads WHERE id = ?",
          [req.params.id],
          (err, result) => {
            if (err) {
              return reject(err);
...
app.post("/convert-customer/:id", async (req, res) => {
  ...
  try {
    ...
    db.query(
      ...
      (err) => {
        if (err) {
          ...
          return res
            .status(500)
            .json({ message: "Error converting to customer" });
        }
        res
          .status(200)
          .json({ message: "Lead converted to customer successfully" });
      }
    );
  } catch (error) {
    console.error("Cloudinary upload error:", error);
    res
      .status(500)
      .json({ message: "Error uploading documents to Cloudinary" });
  }
});
